        ...
        private void ChangeCamHeight()
        {
            if (File.Exists("!Contra_Classic.big") || File.Exists("!Contra_Classic.ctr"))
            {
                if (File.Exists("!Contra_Classic_GameData.big"))
                {
                    ...
                    if ((AspectRatio(x, y) == "16:9") && isGentoolInstalled("d3d8.dll"))
                    {
                        regex = Regex.Replace(File.ReadAllText("!Contra_Classic_GameData.big"), "  MaxCameraHeight = .*\r?\n", "  MaxCameraHeight = " + (camTrackBar.Value - 110) + ".0" + " ;350.0\r\n");
                    }
                    else
                    {
                    ...
                    {
                        var regex2 = Regex.Replace(File.ReadAllText("!Contra_Classic_GameData.big"), "  DrawEntireTerrain = No\r?\n", "  DrawEntireTerrain = Yes\r\n");
                        string read2 = File.ReadAllText("!Contra_Classic_GameData.big", encoding);
                        File.WriteAllText("!Contra_Classic_GameData.big", regex2, encoding);
                    }
                    else
                    {
                        var regex2 = Regex.Replace(File.ReadAllText("!Contra_Classic_GameData.big"), "  DrawEntireTerrain = Yes\r?\n", "  DrawEntireTerrain = No\r\n");
                        string read2 = File.ReadAllText("!Contra_Classic_GameData.big", encoding);
                        File.WriteAllText("!Contra_Classic_GameData.big", regex2, encoding);
                    }
